...
contract vPrime is PendingOwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(address => bool) public isParameterUpdatePending; // Account needs balance recalculating
    ...
    function flagUserForParameterUpdate(address user) external onlyVPrimeController {
        isParameterUpdatePending[user] = true;
        emit UserNeedsUpdateSet(user, msg.sender, block.timestamp);
    }

    function clearUserParameterUpdatePending(address user) internal {
        isParameterUpdatePending[user] = false;
    ...
    */
    ...
    function adjustRateAndCap(address user, int256 rate, uint256 newBalanceLimit) external onlyVPrimeController nonReentrant {
        ...
        clearUserParameterUpdatePending(user);
    }
    ...
    function balanceOf(address account) public view returns (uint256) {
        ...
        if(isParameterUpdatePending[account]) {
            return cp.balance;
        }
    ...
    function _writeCheckpoint(
        ...
        if (pos > 0 && oldCkpt.blockTimestamp == clock()) {
            ...
            ckpts[pos - 1].rate = rate;
            ckpts[pos - 1].balanceLimit = balanceLimit;
        } else {
    ...
    function _unsafeAccess(Checkpoint[] storage ckpts, uint256 pos) private pure returns (Checkpoint storage result) {
        assembly {
            mstore(0, ckpts.slot)
            let baseSlot := keccak256(0, 0x20)
            result.slot := add(baseSlot, mul(pos, 4)) // multiply by 4 slots
        }
    }
