    ...
    RedstoneConsumerNumericBase
{
    ...
    /**
     * @dev Struct to hold the details calculated for an ideal swap.
     */
    struct SwapCalculationDetails {
        uint256 idealSwapAmountIn;  // Amount of the token to be sold
        uint256 idealSwapAmountOut; // Amount of the token to be bought
        address tokenToSell;        // Address of the token to sell
        address tokenToBuy;         // Address of the token to buy
    }
    ...
    /**
     * @dev Calculates the ideal amounts and tokens for a rebalancing swap.
     * @param diffInYValue The value difference between tokenX and tokenY holdings, denominated in tokenY.
     * @param shouldSellX True if tokenX should be sold to buy tokenY, false otherwise.
     * @param priceXOverYScaled The price of tokenX in terms of tokenY, scaled by 10^PRECISION.
     * @param tokenXAddress The address of the contract's tokenX.
     * @param tokenYAddress The address of the contract's tokenY.
     * @return details A SwapCalculationDetails struct containing the swap parameters.
     */
    function _calculateIdealSwapDetails(
        uint256 diffInYValue,
        bool shouldSellX,
        uint256 priceXOverYScaled,
        address tokenXAddress,
        address tokenYAddress
    ) internal pure returns (SwapCalculationDetails memory details) {
        uint256 halfDiffInYValue = diffInYValue / 2;

        if (shouldSellX) { // Sell tokenX, buy tokenY
            details.tokenToSell = tokenXAddress;
            details.tokenToBuy = tokenYAddress;
            details.idealSwapAmountOut = halfDiffInYValue; // We want to receive this much of tokenY
            // Amount of tokenX to sell = idealSwapAmountOut (in Y) / (PriceX_in_Y_unscaled)
            // PriceX_in_Y_unscaled = priceXOverYScaled / (10**PRECISION)
            details.idealSwapAmountIn = FullMath.mulDiv(details.idealSwapAmountOut, 10**PRECISION, priceXOverYScaled);
        } else { // Sell tokenY, buy tokenX
            details.tokenToSell = tokenYAddress;
            details.tokenToBuy = tokenXAddress;
            details.idealSwapAmountIn = halfDiffInYValue; // We want to sell this much of tokenY
            // Amount of tokenX to receive = idealSwapAmountIn (in Y) * PriceX_in_Y_unscaled
            // PriceX_in_Y_unscaled = priceXOverYScaled / (10**PRECISION)
            details.idealSwapAmountOut = FullMath.mulDiv(details.idealSwapAmountIn, priceXOverYScaled, 10**PRECISION);
        }
    }
    ...
    /**
     * @dev Returns the estimated token Y amount from a given token X amount, using an oracle-derived price.
     * @param amountX The amount of token X.
     * @return amountY The estimated amount of token Y.
     */
    function _getTokenYFromTokenX(uint256 amountX) internal view returns (uint256 amountY) {
        uint256 priceFromOracle = getPoolPrice();
        uint160 sqrtPriceX96 = _convertPriceToSqrtPriceX96(priceFromOracle);
        
        int24 tickFromPrice = TickMath.getTickAtSqrtRatio(sqrtPriceX96);
        
        // Ensure amountX fits into uint128 for OracleLibrary.getQuoteAtTick
        if (amountX > type(uint128).max) {
            revert("Amount X exceeds uint128 max");
        }
    ...
    /**
     * @dev Converts a scaled price ratio (price * 10^PRECISION) to sqrtPriceX96 format.
     * @param scaledPriceRatioWithPrecision The price of token0 in terms of token1, scaled by 10^PRECISION.
     * @return sqrtPriceX96 The square root of the price ratio in Q64.96 format.
     */
    function _convertPriceToSqrtPriceX96(uint256 scaledPriceRatioWithPrecision) internal pure returns (uint160) {
        uint256 sqrtOfScaledRatio = UniswapV3IntegrationHelper.sqrt(scaledPriceRatioWithPrecision);
        uint256 calculatedSqrtPriceX96 = FullMath.mulDiv(
            sqrtOfScaledRatio,
            2**96,
            10**(PRECISION / 2) // PRECISION is 20, so 10**(20/2) = 10**10
        );

        if (calculatedSqrtPriceX96 < TickMath.MIN_SQRT_RATIO || calculatedSqrtPriceX96 > TickMath.MAX_SQRT_RATIO) {
            revert("Calculated SqrtPriceX96 out of bounds");
        }
        return uint160(calculatedSqrtPriceX96);
    }

    function getAssetSymbolForAddress(address token) private pure returns (bytes32 symbol){
        if(token == 0x82aF49447D8a07e3bd95BD0d56f35241523fBab1){
            symbol = "ETH";
        } else if(token == 0x3De81CE90f5A27C5E6A5aDb04b54ABA488a6d14E){
            symbol = "PRIME";
        } else {
    ...
    function getPoolPrice() public view returns (uint256) {
        ...
        uint256 oraclePrice0Raw = getOracleNumericValueFromTxMsg(getAssetSymbolForAddress(token0Address));
        uint256 oraclePrice1Raw = getOracleNumericValueFromTxMsg(getAssetSymbolForAddress(token1Address));

        if (oraclePrice1Raw == 0) {
            revert("Price oracle returned zero for token1"); 
        }

        uint8 decimals0 = IERC20Metadata(token0Address).decimals();
        uint8 decimals1 = IERC20Metadata(token1Address).decimals();

        uint256 denominatorValue = oraclePrice1Raw * (10**uint256(decimals0));
        
        if (denominatorValue == 0) {
            revert("Denominator is zero in price calculation");
        }
        
        return (oraclePrice0Raw * (10**uint256(decimals1)) * (10**PRECISION)) / denominatorValue;
    }

    /**
     * @dev Swaps tokens to achieve equal USD values of tokenX and tokenY holdings, if rebalance is needed.
     * @param currentAmountX The current amount of token X held.
     * @param currentAmountY The current amount of token Y held.
     * @param swapSlippage The allowed slippage for the swap, e.g., 500 for 5% (scaled by _DENOMINATOR).
     * @return finalAmountX The amount of token X after potential swap.
     * @return finalAmountY The amount of token Y after potential swap.
     */
    function _swapForEqualValues(
        uint256 currentAmountX,
        uint256 currentAmountY,
        uint256 swapSlippage
    ) internal returns (uint256 finalAmountX, uint256 finalAmountY) {
        finalAmountX = currentAmountX;
        finalAmountY = currentAmountY;

        uint256 valueOfX_in_Y = _getTokenYFromTokenX(currentAmountX);
        bool shouldSellX = currentAmountY < valueOfX_in_Y; 
        uint256 diffInYValue = shouldSellX ? valueOfX_in_Y - currentAmountY : currentAmountY - valueOfX_in_Y;

        if (
            (currentAmountY * _REBALANCE_MARGIN) / 100 < diffInYValue &&
            (valueOfX_in_Y > 0 || currentAmountX == 0) 
        ) {
            uint256 price0Over1Scaled = getPoolPrice(); 
            if (price0Over1Scaled == 0) revert("Oracle price is zero");

            uint256 priceXOverYScaled;
            if (address(tokenX) == pool.token0()) { 
                priceXOverYScaled = price0Over1Scaled;
            } else { 
                priceXOverYScaled = FullMath.mulDiv(10**PRECISION, 10**PRECISION, price0Over1Scaled);
            }
            
            if (priceXOverYScaled == 0) revert("Calculated priceXOverYScaled is zero");

            SwapCalculationDetails memory swapDetails = _calculateIdealSwapDetails(
                diffInYValue,
                shouldSellX,
                priceXOverYScaled,
                address(tokenX),
            ...
            if (swapDetails.idealSwapAmountIn == 0) { 
                return (finalAmountX, finalAmountY);
            }

            // Calculate minAmountOutToReceive explicitly before other operations on this value
            // Ensure swapSlippage <= _DENOMINATOR (e.g. _MAX_SLIPPAGE check in calling function)
            uint256 slippageFactor = _DENOMINATOR - swapSlippage;
            uint256 minAmountOutToReceive = FullMath.mulDiv(swapDetails.idealSwapAmountOut, slippageFactor, _DENOMINATOR);

            IERC20Metadata(swapDetails.tokenToSell).safeApprove(address(swapRouter), 0);
            IERC20Metadata(swapDetails.tokenToSell).safeApprove(address(swapRouter), swapDetails.idealSwapAmountIn);
            
            (uint256 actualAmountSwappedIn, uint256 actualAmountReceivedOut) = _processTokenSwap(
                swapDetails.tokenToSell,
                swapDetails.tokenToBuy,
                swapDetails.idealSwapAmountIn,
                minAmountOutToReceive 
            );

            if (shouldSellX) {
                finalAmountX = currentAmountX - actualAmountSwappedIn; 
                finalAmountY = currentAmountY + actualAmountReceivedOut; 
            } else {
                finalAmountX = currentAmountX + actualAmountReceivedOut; 
                finalAmountY = currentAmountY - actualAmountSwappedIn; 
            }
    ...
    function _depositToUniswap(
        ...
        tokenX.forceApprove(address(positionManager), 0);
        tokenY.forceApprove(address(positionManager), 0);
    }
    ...
    function _afterTokenTransfer(
        ...
        if (from != address(0) && to != address(0)) {
            ...
            if (fromBalance == 0) {
                ...
                // revoke unused approval
                getToken0().forceApprove(address(positionManager), 0);
                getToken1().forceApprove(address(positionManager), 0);
            }
