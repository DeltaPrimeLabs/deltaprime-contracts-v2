...
contract BalancerV2Facet is ReentrancyGuardKeccak, OnlyOwnerOrInsolvent, IBalancerV2Facet {
     ...
     **/
    function joinPoolAndStakeBalancerV2(IBalancerV2Facet.StakeRequest memory request) external nonReentrant onlyOwner noBorrowInTheSameBlock remainsSolvent notInLiquidation {
        ...
        uint256 initialGaugeBalance = IERC20(gauge).balanceOf(address(this));
        {
            ...
            IERC20(pool).approve(address(gauge), 0);
        }
        ...
        for (uint256 i; i < stakedTokensLength; ++i ) {
            if (request.stakedAmounts[i] > 0) {
                ...
                // loop to revoke unused approvals
                request.stakedTokens[i].safeApprove(MASTER_VAULT_ADDRESS, 0);
            }
        }
     ...
     **/
    function stakeBalancerV2(bytes32 poolId, uint256 amount) external nonReentrant onlyOwner noBorrowInTheSameBlock remainsSolvent {
        ITokenManager tokenManager = DeploymentConstants.getTokenManager();
        ...
        uint256 initialGaugeBalance = IERC20(gauge).balanceOf(address(this));
        amount = Math.min(amount, _getAvailableBalance(tokenManager.tokenAddressToSymbol(pool)));
    ...
    **/
    ...
    function claimRewardsBalancerV2(bytes32 poolId) external nonReentrant onlyOwner noBorrowInTheSameBlock remainsSolvent {
        ITokenManager tokenManager = DeploymentConstants.getTokenManager();
