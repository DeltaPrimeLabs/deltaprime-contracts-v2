...
contract UniswapV3Facet is IUniswapV3Facet, AvalancheDataServiceConsumerBase, ReentrancyGuardKeccak, OnlyOwnerOrInsolvent {
    ...
    function mintLiquidityUniswapV3(INonfungiblePositionManager.MintParams memory params) external nonReentrant onlyOwner noBorrowInTheSameBlock remainsSolvent notInLiquidation {
        ...
        {
            require(_getAvailableBalance(token0) >= params.amount0Desired, "Insufficient token0 balance");
            require(_getAvailableBalance(token1) >= params.amount1Desired, "Insufficient token1 balance");
        }
    ...
    function increaseLiquidityUniswapV3(INonfungiblePositionManager.IncreaseLiquidityParams memory params) external nonReentrant onlyOwner noBorrowInTheSameBlock remainsSolvent notInLiquidation {
        (
        ,,
