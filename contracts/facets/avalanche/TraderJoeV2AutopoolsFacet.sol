...
contract TraderJoeV2AutopoolsFacet is ITraderJoeV2AutopoolsFacet, ReentrancyGuardKeccak, OnlyOwnerOrInsolvent {
     ...
     **/
    function stakeTokenTraderJoeV2Autopool(ITraderJoeV2Autopool.StakingDetails memory stakingDetails) private nonReentrant onlyOwner noBorrowInTheSameBlock remainsSolvent {
        ...
        stakingDetails.amount0Desired = Math.min(_getAvailableBalance(stakingDetails.token0Symbol), stakingDetails.amount0Desired);
        stakingDetails.amount1Desired = Math.min(_getAvailableBalance(stakingDetails.token1Symbol), stakingDetails.amount1Desired);
    ...
    function unstakeTokenTraderJoeV2Autopool(ITraderJoeV2Autopool.UnstakingDetails memory unstakingDetails) private nonReentrant onlyOwnerOrInsolvent noBorrowInTheSameBlock notInLiquidation {
        ...
        uint256 amount1Unstaked;
        {
            ...
            IERC20 depositToken1 = IERC20(tokenManager.getAssetAddress(unstakingDetails.token1Symbol, false));
            {
                ...
                unstakingDetails.liquidity = Math.min(_getAvailableBalance(unstakingDetails.vaultTokenSymbol), unstakingDetails.liquidity);
