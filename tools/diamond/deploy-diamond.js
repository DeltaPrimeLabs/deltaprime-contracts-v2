...
async function deployDiamond(hardhatConfig = undefined, mock = true) {
    const accounts = await ethers.getSigners()
    const contractOwner = hardhatConfig === undefined ? accounts[0] : hardhatConfig.deployer;

    // deploy DiamondCutFacet or MockDiamondCutFacet based on mock parameter
    const diamondCutFacetName = mock ? 'MockDiamondCutFacet' : 'DiamondCutFacet';
    const diamondCutFacet = await deployContract(diamondCutFacetName, [], {}, hardhatConfig);

    console.log(`${diamondCutFacetName} deployed:`, diamondCutFacet.address)
